...
TLD::TLD()
{
    boxdata=main.subscribe("boxdata",1,&TLD::chatterCallback,this);
}
TLD::TLD(int sss){
  read(sss);
}
void TLD::chatterCallback(const xm_tld::box& msg)
{
  rightbox.x=msg.x;
  rightbox.y=msg.y;
  rightbox.width=msg.width;
  rightbox.height=msg.height;
  tldboxfile=true;
}
void TLD::read(int sss){
...
void TLD::processFrame(const cv::Mat& img1,const cv::Mat& img2,vector<Point2f>& points1,vector<Point2f>& points2,BoundingBox& bbnext,bool& lastboxfound, bool tl, FILE* bb_file){
  ...
  if (tracked){
      ...
      /*if(lastboxfound == false&&boxfile==true&&rightbox.width!=0)
      {
	bbnext.x=rightbox.x+2*(rightbox.width)/7;
	bbnext.y=rightbox.y+(rightbox.height)/5;
	bbnext.width=2*(rightbox.width)/5;
	bbnext.height=(rightbox.height)/3;
	lastconf=1;
	lastboxfound = true;
      }*/
